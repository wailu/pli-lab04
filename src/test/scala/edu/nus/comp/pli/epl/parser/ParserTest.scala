...
class ParserTest extends FlatSpec {
  ...
  "1) The parser" should "parse an int value" in {
    assert(
      parse("123") == Num(123)
    )
  }

  "2) The parser" should "parse a bool value" in {
    assert(
      parse("true") == Bool(true)
    )
  }

  "3) The parser" should "parse a bool value" in {
    assert(
      parse("false") == Bool(false)
    )
  }

  "4) The parser" should "parse a binary operation" in {
    assert(
  ...
  "5) The parser" should "parse a binary operation" in {
    assert(
      parse("1+-true") == Bin(Add, Num(1), Ury(Minus, Bool(true)))
    )
  }

  "6) The parser" should "parse a binary operation" in {
    assert(
      parse("1--true") == Bin(Sub, Num(1), Ury(Minus, Bool(true)))
    )
  }

  "7) The parser" should "parse a binary operation" in {
    assert(
      parse("1+2*3-4 <= 5") ==
        Bin(
          Lte,
  ...
  "8) Brackets" should "parse a binary operation" in {
    assert(
      parse("(1+2)*(4-3)") ==
        Bin(
          Mul,
  ...
  "9) the parser" should "parse a binary operation" in {
    assert(
      parse("1 <= 2 & 4 == 5") ==
  ...
  "10) the parser" should "parse a binary operation" in {
    assert(
      parse("2^3") ==
        Bin(Pow,Num(2),Num(3))
    )
  ...
  "11) the parser" should "parse a binary operation" in {
    assert(
  ...
  "12) the parser" should "parse a binary operation" in {
    assert(
      parse("false == 10 > 6") ==
  ...
  "13) the parser" should "parse a binary operation" in {
    assert(
  ...
  "14) the parser" should "parse a binary operation" in {
    assert(
      parse("10 == 3 + 7") ==
  ...
  "15) the parser" should "parse a binary operation" in {
    assert(
      parse("1+2*3+4") ==
  ...
  "16) the parser" should "parse a binary operation" in {
    assert(
      parse("(1+2)*3") ==
        Bin(Mul,Bin(Add,Num(1),Num(2)),Num(3))
    )
  }
  "17) the parser" should "parse a binary operation" in {
    assert(
      parse("(1+2)*(4-3)") ==
  ...
  "18) the parser" should "parse an unary operation" in {
    assert(
      parse("---5") ==
  ...
  "19) the parser" should "parse an unary operation" in {
    assert(
      parse("--5") ==
  ...
  "20) the parser" should "parse a binary operation" in {
    assert(
      parse("false == (!true) | false & (2+3 ==4)") ==
  ...
  "21) the parser" should "parse an if-else statement" in {
    assert(
  ...
  "22) the parser" should "parse an if-else statement" in {
    assert(
  ...
  "23) the parser" should "parse a binary operation" in {
    assert(
      parse("(2+3)*4/2-1") ==
        Bin(Sub,Bin(Div,Bin(Mul,Bin(Add,Num(2),Num(3)),Num(4)),Num(2)),Num(1))
    )
  }
  "24) the parser" should "parse an if-else statement" in {
    assert(
  ...
  "25) the parser" should "parse a binary operation" in {
    assert(
      parse("2^true") ==
